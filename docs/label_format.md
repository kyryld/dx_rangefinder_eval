...
| Version | Change |
|---|---|
| 1 | Initial version. Object class stored under the key `"class"`. |
| 2 | Renamed `"class"` → `"class_name"`. No other changes. |
...
|---|---|---|---|
| `format_version` | integer | yes | Schema version. Currently `2`. |
...
| `class_name` | string | yes | yes | One of `person`, `vehicle`, `building`. |
...
| `confidence_detection` | number 0–1 | no | **strongly recommended** | Detection confidence score. Used for AP calculation. **If absent in all predictions, AP is reported as N/A** — ranking without scores is not a meaningful measurement. |
| `confidence_distance` | number 0–1 | no | optional | Confidence in the distance estimate. Reserved for future metrics. |
...
- **Removing or renaming existing fields**, or changing their semantics,
  **requires incrementing `format_version`** and updating the scorer.
- The current scorer validates that `format_version == 2` and raises a clear
  error for other versions, including a migration hint for v1 files.
